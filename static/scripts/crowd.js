...
function createEventListenerForCrowd() {
    ...
    currentDonationForCrowd = getPreDonationIncome() * 0.1
 
	sliderElement.noUiSlider.on('update', () => {
        const donatedAmount = getDonatedAmount();
        if (donatedAmount != currentDonationForCrowd){
            drawCrowdofPeople();
        }
    })
}
...
function drawCrowdofPeople() {
    ...
    if (sliderElement.classList.contains("active")){
        donationAmount = getDonatedAmount();
    }
    else {
        donationAmount = getPreDonationIncome() * 0.1;
    }
    currentDonationForCrowd = donationAmount;
